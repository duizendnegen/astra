...
At Lambda module initialisation in `skeleton.ts`, all AWS SDK v3 clients (`DynamoDBClient`, `S3Client`, and `SSMClient`) SHALL be wrapped with `AWSXRay.captureAWSv3Client()` before being used. This causes every AWS SDK call made through these clients to appear as a named sub-segment in the X-Ray trace automatically.
