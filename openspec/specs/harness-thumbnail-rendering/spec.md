...
The system SHALL provide a `renderPatch(result: WordResult, opts: RenderOpts) => Buffer` function in `test-harness/render-patch.ts` that produces a PNG buffer using `node-canvas` and `d3-geo`. It SHALL NOT import from `frontend/src/renderer.ts`. The visual style SHALL match the frontend closely: same star colour palette, same magnitude-to-radius formula, same edge line style.
...
The renderer SHALL use `d3.geoStereographic` centred on `patchRA` / `patchDec` and scaled to fit `patchRadiusDeg` within the image bounds. This is the same projection used by the frontend renderer.
