...
The four new preview words SHALL be added to `test-harness/words.ts`: banana (Category A), bunny (Category A), tree (Category A), love (Category E). These words SHALL be valid entries in the `words` array so the `--words` filter in `run.ts` accepts them.
