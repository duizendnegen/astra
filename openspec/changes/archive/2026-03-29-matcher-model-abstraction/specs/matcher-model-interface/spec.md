...
- **WHEN** the vertex model default for `distanceThreshold` is 0.10 and the caller passes `{ model: 'vertex', distanceThreshold: 0.07 }`
...
The `vertex` model SHALL apply a Gaussian proximity reduction to the segment distance, with separate bonus magnitudes for degree-1 endpoint vertices (`VERTEX_BONUS_ENDPOINT`) and degree-2+ joint vertices (`VERTEX_BONUS_JOINT`). Its `spreadScore` function SHALL return 0.
