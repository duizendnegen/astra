...
export function renderPatch(result: WordResult, opts: RenderOpts): Buffer {
  ...
  // Constellation edges — connect the actual matched stars, not the ideal skeleton positions
  if (result.edges.length > 0 && result.constellationStarIds.length > 0) {
    const starById = new Map(result.patchStars.map(s => [s.id, s]));
    ctx.strokeStyle = 'rgba(100, 160, 255, 0.55)';
    ctx.lineWidth = 1;
    for (const [i, j] of result.edges) {
      const sa = starById.get(result.constellationStarIds[i]);
      const sb = starById.get(result.constellationStarIds[j]);
      if (!sa || !sb) continue;
      const pa = proj([sa.ra, sa.dec]);
      const pb = proj([sb.ra, sb.dec]);
      if (!pa || !pb) continue;
      ctx.beginPath();
